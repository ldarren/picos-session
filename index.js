...
Session.prototype = {
	...
	fork(api){
		const sess=new Session()
		sess.clone(api, this.sigslot, this.type, this.models, this.time, this.userData)
	...
	get(key){
		const m=this.models; return m[key] = m[key] || {}
	},
	getArray(key){
		const m=this.models; return m[key] = m[key] || []
	},
	getSet(key){
		const m=this.models; return m[key] = m[key] || new Set
	},
	getWeakSet(key){
		const m=this.models; return m[key] = m[key] || new WeakSet
	},
	getMap(key){
		const m=this.models; return m[key] = m[key] || new Map
	},
	getWeakMap(key){
		const m=this.models; return m[key] = m[key] || new WeakMap
	},
	set(key, value){
